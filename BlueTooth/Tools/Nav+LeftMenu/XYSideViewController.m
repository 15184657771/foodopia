...
- (instancetype)initWithSideVC:(UIViewController *)sideMenuVC currentVC:(UIViewController *)currentMainVC
{
    ...
    if (self = [super init]) {
        ...
        _sideVC = sideMenuVC;
        _sideContentOffset = XYScreenWidth * 2 / 3;
...
- (void)panGesture:(UIPanGestureRecognizer *)pan
{
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (tabBarVCCenter.x >= viewStartCenterPoint.x  && (viewStartCenterPoint.x + _sideContentOffset) >= tabBarVCCenter.x) {
            ...
            if (movePoint.x > 0) {
                _currentVC.view.transform = CGAffineTransformMakeScale(1 - 0.1 * (movePoint.x/_sideContentOffset), 1 - 0.1 * (movePoint.x/_sideContentOffset));
                [_currentVC.view.layer setCornerRadius:50 * movePoint.x/_sideContentOffset];
            } else {
                _currentVC.view.transform = CGAffineTransformMakeScale(0.9 + 0.1 * (fabs(movePoint.x)/_sideContentOffset), 0.9 + 0.1 * (fabs(movePoint.x)/_sideContentOffset));
                [_currentVC.view.layer setCornerRadius:50 * (1 - fabs(movePoint.x)/_sideContentOffset)];
            }
            [_currentVC.view.layer setMasksToBounds:YES];
        }else if (viewStartCenterPoint.x > tabBarVCCenter.x){
            _currentVC.view.center = self.view.center;
            CGPoint point = self.view.center;
            point.x = point.x - (_sideContentOffset / 2);
            _sideVC.view.center = point;
            _currentVC.view.transform = CGAffineTransformMakeScale(1, 1);
            [_currentVC.view.layer setCornerRadius:0];
            [_currentVC.view.layer setMasksToBounds:YES];
            ...
            _currentVC.view.transform = CGAffineTransformMakeScale(0.9, 0.9);
            [_currentVC.view.layer setCornerRadius:50];
            [_currentVC.view.layer setMasksToBounds:YES];
        }
